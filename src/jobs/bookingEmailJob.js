...
async function runBookingEmailJob() {
  ...
  try {
    ...
    const endedBookings = await Booking.find({ end: { $lte: now }, bookingStatus: 'active' })
      .populate("user", "email")
      .populate("vault")
      .lean();
    ...
    for (const b of endedBookings) {
      ...
      // Mark booking as expired instead of deleting it to keep history
      await Booking.findByIdAndUpdate(b._id, { bookingStatus: 'expired' });
